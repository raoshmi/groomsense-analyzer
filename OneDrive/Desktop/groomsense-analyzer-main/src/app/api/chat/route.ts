...
export async function POST(req: NextRequest) {
  try {
    const { message, userProfile, analysisHistory, lang = 'en' } = await req.json();
...
Current Groom Score: ${userProfile.latestScore || 'not analyzed yet'}
    ...
    const languageInstruction = lang === 'hi'
      ? "IMPORTANT: You MUST converse and answer the user entirely in natural Devanagari Hindi or a cool modern Indian Hinglish style. Make it extremely encouraging, highly relatable, and easy to read."
      : "Converse and answer in natural English.";
